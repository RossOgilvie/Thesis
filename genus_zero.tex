...
\begin{figure}
    ...
    \caption{The image of two harmonic maps of the form \eqref{eqn:genus zero simple map} with $δ=π/4$ (black) and $3π/4$ (blue). The two images are congruent and share a common circle $\Set{\exp(ω σ_2)}{ω \in \R}$, drawn in red. In fact all tori of the form \eqref{eqn:genus zero simple map} are tangent along this circle. It is the circle towards which the torus in Figure \ref{fig:genus0 plot pi32} is tending. Every point of $\S^3$ except those on this circle belongs to exactly one image, so varying the parameter $δ$ sweeps out all of $\S^3$.}
