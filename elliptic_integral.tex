...
Above, we computed the limits of $F_0(x;k)$ and $E_0(x;k)$ as $x \to \infty$, but because both functions are odd, there is no way to extend them to single valued functions on $\RInf$. Instead we must extend these functions analytically. Let $\tilde{x}\in\R$ and consider the covering map $\R \to \RInf$ given by
\[
...
\end{align*}
Manipulating the integrand into a form that is plainly analytic demonstrates that $\tilde{E}$ is as well. One could reasonably ask how to compute the values of these extended functions, and the answer is quite simple. Define the winding number $W : \R \to \Z$ of a number $x$ to be the integer $W(x)$ such that $-π < x - 2πW(x) < π$. Let $n=W(\tilde{x})$ and write $\tilde{x} = 2πn + y$. Then
\begin{align*}
\tilde{F}(\tilde{x};k)
...
\end{align*}
Let $\tilde{F}$ and $\tilde{E}$ be the lifts to the universal cover of $F_0$ and $E_0$ respectively. We will give concrete expressions for these functions below.

Recall that the period of $F_0(x)$ is $2K'$ and that of $E_0(x)$ it is $2(K' - E')$. To give the extension of either of these functions to the universal cover, it is sufficient to give an expression in each coordinate chart. On the universal cover, the extension will differ from the pullback by some number of periods.
...
&=  K' - \int_0^{x'} \frac{ds}{\sqrt{(1+s^2)(s^2+k^2)}}.
\end{align*}
So we have arrived at a single formula for all values of $x'$, namely
\[
...
The definition and check for the integral of the second kind is entirely similar.
\[
E_1(x';k):=
\begin{cases}
E_0((x')^{-1}; k)         & \text{ for } x' > 0 \\
K'-E'                                                   & \text{ for } x' = 0 \\
2(K'-E') + E_0((x')^{-1}; k)   & \text{ for } x' < 0
\end{cases}
\]
After transforming the integrals in the same way, we find ourselves faced with the formula, for all $x'$,
\[
E_1(x';k) = (K'-E') - \int_0^{x'} s^{-2}\bra{ \sqrt{\frac{s^2 + k^2}{1 + s^2}} - k }\;ds.
\]
On its face, the integrand appears not to be analytic because of the $s^{-2}$. However the difference in the bracket vanishes quadratically in $s$. To see this, use the square root estimate
\[
...
\tilde E(\tilde{x};k) =
\begin{cases}
E_0(x; k) + 2(K'-E')n             & \text{ for } \tilde x \in (2πn - π, 2πn + π) \\
E_1(x'; k) + 2(K'-E')n     & \text{ for } \tilde x \in (2πn, 2πn + 2π).
\end{cases}
\]
