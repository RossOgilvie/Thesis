...
\end{align*}
Manipulating the integrand into a form that is plainly analytic demonstrates that $\tilde{E}$ is as well. One could reasonably ask how to compute the values of these extended functions, and the answer is quite simple. Write $\tilde{x} = 2π n + y$ for $n$ the greatest integer less than $\tilde{x}$. Then
\begin{align*}
\tilde{F}(\tilde{x};k)
