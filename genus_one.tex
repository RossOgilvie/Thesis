...
The difference between a genus one marked curve and a spectral curve is that the latter possesses differentials that satisfy the closing conditions, Conditions \ref{P:closing}, in addition to satisfying Conditions \ref{P:real curve}--\ref{P:periods}. For any genus $g$, every marked curve of $\mathcal{C}_g$ admits differentials satisfying Conditions \ref{P:real curve}--\ref{P:imaginary periods}. In the particular case of marked curves of genus one, every curve further has differentials that have integral periods (Condition \ref{P:periods}). But not every curve admits differentials that meet the closing conditions. In Section \ref{sec:closing conditions}, we produce two functions $S$ and $T$ on $\mathcal{C}_1$ such that a curve belongs to $\mathcal{S}_1$ precisely when $S$ and $T$ are rationally valued.

The key to finding $\mathcal{\tilde{S}}_1$, the preimage of $\mathcal{S}_1$ in $\mathcal{\tilde{C}}_1$, is the constuction of coordinates on $\mathcal{\tilde{C}}_1$ such that $p = S$ and $q = T$ are coordinates. In order to be able to take the quotient, we also desire the group of covering transformation to act as translations in these coordinates. To satisfy the second requirement, we will construct
\[
...
\]
In Lemma \ref{lem:T_graph}, we divide $\mathcal{\tilde{C}}_1$ into two pieces and show that for $p \leq 1$ we may use $(p,q,k,\tilde{U})$ as coordinates, and that for $p \geq 1$ we may use $(p,q,k,\tilde{V})$. In a neighbourhood of $p=1$ it is possible to use either set of coordinates.
If $\mathcal{\tilde{C}}_1(p,q)$ is the subspace of $\mathcal{\tilde{C}}_1$ where the coordinates $p$ and $q$ are fixed, the space $\mathcal{\tilde{S}}_1$ is then the union of points whose $p$ and $q$ coordinates lie in $\Q$,
\[
\mathcal{\tilde{S}}_1 =  \coprod_{p \in \Q^+, q\in\Q} \mathcal{\tilde{C}}_1(p,q).
\]

As the covering transformations do not change $p$, we may consider the quotient on the $\mathcal{G}$-invariant submanifold $\mathcal{\tilde{C}}_1(p)$ of $\mathcal{\tilde{C}}_1$ where $p$ is fixed.
When the transformation $\tilde{λ}$ acts on a point of $\mathcal{\tilde{C}}_1$, it shifts its $q$-coordinate by $p-1$ (Lemma \ref{lem:T shift}).
...
\]
where $\tilde{X}$ stands for either $\tilde{U}$ or $\tilde{V}$ depending on the magnitude of $p$. The moduli space $\mathcal{S}_1(p)$ is therefore the subset where $[q] \in \Q/(p-1)\Z$. For $p=1$, by contrast, Theorem \ref{thm:topology_curves_p1} shows that
\[
\mathcal{C}_1(1)
...
In either case, we have a foliation of $\mathcal{C}_1(p)$ such that $\mathcal{S}_1(p)$ is a dense collection of leaves. Though the topology of the components `jumps' from strips to annuli to at $p=1$, we can understand the foliation as changing continuously in $p$. As $k$ is unaffected by the quotient, we may leave it aside and consider only $(q,\tilde{X}) \in \R \times \R$ in Figure \ref{fig:level set quotient}. For each $p$, $\mathcal{G}$ acts as translations $(q + (p-1)\Z, \tilde{X} + π\Z)$. Rotate each plane so that these translations are in the vertical direction. The lines of constant $q$ now make an angle $\atan \tfrac{p-1}{π}$ to vertical. We may therefore quotient this space into a cylinder, and for $p\neq 1$ the lines of constant $q$ become helices on this cylinder. The parameter $p$ changes the slope of helices. When $p=1$ however the line of constant $q$ are in the direction of translation and close into circles. This exceptional case of $p=1$ is the transition from left-handed to right-handed helices.
...
The proof of the two Theorems \ref{thm:topology_curves} and \ref{thm:topology_curves_p1} therefore proceeds by the construction, in several stages, of the coordinates adapted to the closing conditions and the action of the covering transformations.
...
We then use the implicit function theorem to prove that the level sets of $S$ and $\tilde{T}$, denoted $\mathcal{\tilde{C}}_1(p,q)$, are graphs and foliate the space $\mathcal{\tilde{C}}_1$ (Lemma \ref{lem:T_graph}). This allows for an explicit description of $\mathcal{\tilde{S}}_1$, the preimage of $\mathcal{S}_1$ in $\mathcal{\tilde{C}}_1$.
...
\begin{diagram}
    ...
    \uTo  &&  &&  \uTo  \\
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    \mathcal{\tilde{S}}_1(p) =  \coprod_{q\in\Q} \mathcal{\tilde{C}}_1(p,q)  &&  \rOnto^{\mathcal{G}}  && \mathcal{S}_1(p)
\end{diagram}
\]
...
\maketikzfigure{The $ζ$-plane, with points marked in blue. The black labels are their images under $f$\label{fig:zeta plane}. The red line is the circle through the branch points.}{tikz/zeta_plane}
...
In analogy to Lemma \ref{lem:mathcal tilde A}, we define coordinates $\tilde{U}$ and $\tilde{V}$ on $\mathcal{\tilde{C}}_1$ to be
\begin{align*}
U = \tan \frac{\tilde{U}}{2},       &\quad
    U' = \cot \frac{\tilde{U}}{2},  \\
V = \tan \frac{\tilde{V}}{2},       &\quad
...
These coordinates $\tilde{U}$ and $\tilde{V}$ can be used interchangably with $\tilde{u}$ and $\tilde{v}$. Consider for example the coordinates $(p,k,\tilde{u},\tilde{v})$ on $\mathcal{\tilde{C}}_1$ defined in Lemma \ref{lem:mathcal tilde A}.
...
\]
so this is indeed a valid change of coordinates. The same is true for the coordinates on $\mathcal{\tilde{C}}_1(p)$ provided by Lemma \ref{}. For $p \leq 1$, we may change coordinates from $(p,q,k,\tilde{u})$ to $(p,q,k,\tilde{U})$, and for $p \geq 1$ from $(p,q,k,\tilde{v})$ to $(p,q,k,\tilde{V})$.
...
Translating this into a statement about $\Mat_2^*\Z$, given a tuple of spectral data $(Σ,Θ^1,Θ^2)$ in $\mathcal{M}_1(1,q)$, we may write $Θ^i = a_i Ψ^E + l_i Ψ^P$ for integers $a_i$ and $l_i$. One is free to vary $Σ$ within $\mathcal{S}_1(1,q)$, but the effect of looping around on the differentials is
\begin{align*}
\begin{pmatrix}
Θ^1 \\ Θ^2
\end{pmatrix}
...
\end{pmatrix}.
\end{align*}
Thus the angle parameter has become $π-δ$. As an aside, as seen in Figure \ref{fig:genus zero maps} this angle parameter determines the image of the harmonic map up to an $SO(4)$ rotation of $\S^3$. In particular, these maps $g$ and $g^{-1}$ have congruent images.
