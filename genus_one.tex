...
The proof of the two theorems therefore proceeds by the construction, in several stages, of covering spaces and parametrisations adapted to the closing conditions.
...
We shall use this rescaled coordinate $U$, and likewise $V = \sqrt{k} v$, to construct coordinates on $\mathcal{\tilde{A}}_1$ such that the covering transformations are simply translations. The fact that these should be translations is motivated by the fact that translating the line $\R$ rotates the quotient $\R/\Z$.

In analogy to Lemma \ref{lem:mathcal tilde A}, we define coordinates $\tilde{U}$ and $\tilde{V}$ on $\mathcal{\tilde{A}}_1$ to be
\begin{align*}
U = \tan \frac{\tilde{U}}{2},       &\quad
    U' = \cot \frac{\tilde{U}}{2},  \\
V = \tan \frac{\tilde{V}}{2},       &\quad
...
First let us use the coordinate $(p,k,\tilde{U},\tilde{V})$ on $\mathcal{\tilde{A}}_1$ to find the group $\mathcal{G}$ of covering transformations $\mathcal{\tilde{A}}_1 \to \mathcal{C}_1$. We can now divide the covering transformations into two types: those that pushforward to $\mathcal{A}_1$ to give the identity and those that pushforward to give $λ$. Recall that the projection of the universal cover $\mathcal{\tilde{A}}_1$ to $\mathcal{A}_1$ in part reads $U = \tan \tilde{U}/2$. This is the standard covering of the circle by $\R$, so the covering transformations that push forward to the identity are simply
\[
...
As $u,v$ are both positive, it must be that $\tilde{u} \in (2πn, 2πn + π)$ and $\tilde{v} \in (2πm, 2πm + π)$ for some integers $n$ and $m$. Hence $\tilde{U}$ and $\tilde{V}$ also lie in those two intervals respectively. Under applying the transformation $\tilde{λ}$, $\tilde{U}$ will have been translated by $π$ and so lie in $(2π(n+1) - π, 2π(n+1))$. In other words its winding number has increased by $1$. The same can be said for $\tilde{V}$. As $\tilde{u}$ and $\tilde{v}$ have the same winding numebrs as $\tilde{U}$ and $\tilde{V}$, combining the effect of $λ$ on $T_0$ with the change of winding number in \eqref{eqn:tilde T computable} shows
\begin{align*}
\tilde{T} \circ \tilde{λ} - \tilde{T}
