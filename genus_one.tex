...
\section{Differentials of the Elliptic Spectral Curve}
...
\int_{f(B)} e &= 2\iu(K'-E')
\end{align*}
where $K$ and $E$ are the complete elliptic integrals of the first and second kind, and the prime denotes not the derivative but instead the elliptic complement. The elliptic complement is by definition $k' = \sqrt{1-k^2}$, and $K'(k) = K(k')$ and $E'(k) = E(k')$. Further properties of elliptic integrals may be found in Appendix \ref{chp:Elliptic Integrals}. Note that $λ$ is the sum of $e$ and an exact differential and so has the same periods as $e$.
...
\]
The other derivatives of elliptic integrals are calculated in Section \ref{sec:Derivatives}. Equipped with these tools, the calculation of the derivatives of $T_0$ is mechanical if tedious.
