...
To derive formulae that work on the other coordinate charts without re-expending the quantity of ink it has taken to get $T_0$, we draw on results from appendix REF \todo{ref} in which we establish how to extend some elliptic integrals analytically at infinity. In particular, we recall the notation $F_0$ and $E_0$ for the imaginary parts of $F$ and $E$ applied to imaginary arguments. We also will be making use of $F_1$ and $E_1$, the analytic extensions of $F_0$ and $E_0$ from $(0,\infty)$ to $(0,\infty)\cup \{\infty\} \cup (-\infty, 0)$.

Let us consider the coordinate patch $(p,k,u,v')$. We rearrange the above to give
\[
...
The same trick in $u$ works if one wishes to write a formula for a branch cut of $T$ that is defined on $(p,k,u',v)$ or $(p,k,u',v')$. As mentioned, it is sufficient to use three coordinate patches to cover $\mathcal{A}$, so let
\[
...
\end{align*}
Building on the definitions in appedix REF \todo{REF}, it is straightforward to lift $T$ to a single valued function $\tilde{T}$ on $\mathcal{\tilde{A}}$. One may simply plug the premade universal covers versions of $F_0,E_0$, etc into the formula to define
\[
...
\end{cases}
\]
where the formulas on the right agree with one another whereever two are both defined. Thus to do computations with the function $\tilde{T}$, one can instead work with a function downstairs on $\mathcal{A}$.
...
How does the value of $T$ change when this transformation is precomposed? To see this, it is useful to consider the situation in the $ζ$-plane. Suppose for concreteness that $μ$ is in the upper unit circle and $ν$ is in the lower unit circle.
...
\begin{lem}
For $p = 1$, $\mathcal{S}(p)$ is topologically to
\[
\Q \times (0,1) \times \bra{\R/2π\Z}.
\]
\end{proof}
In this special case, we see that the action does not identify path components with different values of $q$; in fact $p-1$ is zero, so $q$ is fixed. Consequently, the shifting action identifies values of $\tilde{x}$ that differ by $2π$, resulting in annuli.
\begin{proof}
\end{lem}
