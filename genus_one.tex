...
\]
disagrees with eqn \eqref{eqn:roots_cross_ratio} in this limit, which rules out this correspondence and the three others with the same cross ratio. Finally, our choice of correspondences takes the unit disk to the right half plane. There is only one other correspondence that has all three of these properties, namely
\[
  \begin{array}{l || c|c|c|c}
    ζ & β & \cji{β} & α & \cji{α} \\
    \hline
...
Recall eqn \eqref{eqn:def_e}, the definition of $e$. It is real with respect to $\tilde{ρ}(z) = -\bar{z}$, but it has a pole at $z=\infty$ ($ζ=ν$), which not allowed. This pole can be moved to $ζ=\infty$ by adding an exact differential. We assert that
\[
...
\]
(compare to \eqref{eqn:f_inv_2}). Thus the construction of $f^{-1}$ proceeds in two steps; first find $z_0$, then determine the correct scaling factor. Note the following trick using cross ratios.
\[
\abs{\frac{α-1}{α+1}}
...
\]
It follows from eqn \eqref{eqn:f_2} that
\[
\iu v = f(-1)
