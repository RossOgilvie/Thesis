...
\begin{proof}
The method of proof is similar to that of Lemma \ref{lem:closing_conds}, but the construction must be refined a little. Let $Ψ^E$ be as described in that lemma. Specifically recall that on a marked curve $Σ(α,β)$, if we write $S(α,β) = n/m$ as a rational number in lowest terms then define
\[
