...
\maketikzfigure{Sketch of some level sets of $\tilde{T}$ on a cross-section of $\mathcal{\tilde{A}}_1$. The transformation $\tilde{λ}$ acts as a translation in the given direction. It take level sets to level sets.}{tikz/universal_level_sets}
...
At a point $(ζ,η)$ over the unit circle in $Σ$, we have that the value of $η$ is $\pm ζ\abs{ζ-α}\abs{ζ-β}$. lgebraically, $η^+(ζ) = ζ\abs{ζ-α}\abs{ζ-β}$.
Thus there is a notion of the `positive' unit circle, the one on which $η$ is positive over $ζ=1$.
we define the function $η^+$ to be the value of $η$.
