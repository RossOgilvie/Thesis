...
\]
Thus we have determined two circles that $z_0$ lies on. These two circles intersect in two points: $z_0$ and $-\conj{z_0}$. The precise formulae are
\[
...
\labelthis{eqn:Teqn}
\end{align*}
where we have omitted the elliptic modulus $k$ in the complete elliptic integrals, ie $K=K(k)$ and $E= E(k)$.

These coordinates show the topology of $\mathcal{A}_1$.
They embed $\mathcal{A}_1$ into $\R^+\times(0,1)\times T^2$ in the following way.
...
The justification of this definition of $\mathcal{\tilde{A}}_1$ precedes in two steps. First, we have already observed $\mathcal{A}_1$ is embeds in to $\R^+ \times (0,1) \times T^2$. The universal cover of this larger space is $\R^+\times(0,1)\times\R^2$, with the covering map given above using the standard half-tan mapping of $\R$ to $\S^1$. The second step is to recall that $\mathcal{A}_1$ is the complement of the hyperplane $u-v = 0$. When pulled back to the universal cover, this hyperplane becomes a collections of hyperplanes $u-v \in 2π\Z$. Thus we may take a simply connected region of the complement $u-v \not\in 2π\Z$ to cover $\mathcal{A}_1$ and this is $\mathcal{\tilde{A}}_1$ above.

It is straightforward to lift $T_0$ to a single valued function $\tilde{T}$ on $\mathcal{\tilde{A}}_1$. Recall the defintions of $F_0$ and $E_0$ from \ref{defn:F0 and E0}:
\[
...
\]
To actually compute the value of this function, it is simply a matter of substituting the correct expression for $\tilde{F}$ or $\tilde{E}$.
...
% + \left\{\frac{w(\iu u)}{u-v} - ku \right\} \right] \\
%%%%%%%%%%%%%%%%%%%%%%%%
&= 4p \left[ 2EK' - 2K(K'-E') \right]\Wind(\tilde{v})
- 4 \left[ 2EK' - 2K(K'-E') \right]\Wind(\tilde{u}) + T_0(p,k,u,v)\\
%%%%%%%%%%%%%%%%%%%%%%%%
\tilde{T}(p,k,\tilde{u},\tilde{v})
&= 2\bra{ p\Wind(\tilde{v}) - \Wind(\tilde{u}) } + T_0(p,k,u,v),
\label{eqn:tilde T computable}
\end{align*}
using Legendre's relation (see Section \ref{sub:Legendre's Relation}).
Thus to do computations with the function $\tilde{T}$, one can continue to work with the function $T_0$ downstairs on $\mathcal{A}_1$ and keep track of the winding numbers. In particular, $T \in \Q$ if and only if $\tilde{T} \in \Q$. In the following section, we will find the level sets of $\tilde{T}$ in $\mathcal{A}_1(p)$.
...
In the interest of having manageable formulae, we recall the definition of $w'(\iu t)^2 = (1 + t^2)(k^2 + t^2)$. As $F(z;k)$ and $E(z;k)$ are parameter integrals in $z$, we have that
\[
...
\]
The other derivatives of elliptic integrals are calculated in Section \ref{sub:Derivatives}. Equipped with these tools, the calculation of the derivatives of $T_0$ is mechanical if tedious.
...
\begin{proof}
We shall prove this statement by showing that the two functions are in fact always positive. We will draw on several inequalities that are explained in Section \ref{sub:Inequalities}. The first step is to eliminate $E$. We apply the crude estimate that $K>E$, from \eqref{eqn:K_E_values}, and also the assumption that $p\geq 1$ to simplify
\begin{align*}
U(p,k,u,v)
...
\begin{lem}
\label{lem:T_graph}
If $p \leq 1$ then there is a diffeomorphism between $\tilde{\mathcal{A}_1}(p)$ and $\{(q,k,\tilde{u}) \in \R\times(0,1)\times\R \}$, such that for fixed $q_0$, the level set $\tilde{T} = q_0$ is $\{(q_0,k,\tilde{u})\}$.

Likewise, if $p \geq 1$ then there is a diffeomorphism between $\tilde{\mathcal{A}_1}(p)$ and $\{(q,k,\tilde{v}) \in \R\times(0,1)\times\R \}$, such that for fixed $q_0$, the level set $\tilde{T} = q_0$ is $\{(q_0,k,\tilde{v})$.
...
\end{align*}
which holds for $\tilde{u} \not\in π + 2π\Z$. As witnessed in Lemma \ref{lem:deriv no zeroes}, $U$ is never zero, and neither are the other three factors present. Hence $\partial \tilde{G} / \partial \tilde{u}$ is never zero on this open set. It remains to check it does not vanish when $\tilde{u} \in π + 2π\Z$. Observe
\begin{align*}
...
\[
2π\iu (T_0 \circ λ - T_0) = \bra{ p\int_{γ'_-} Θ^P - \int_{γ'_+} Θ^P } - \bra{ p\int_{γ_-} Θ^P - \int_{γ_+} Θ^P } = p(-2π\iu) - (-2π\iu) = 2π\iu (1-p).
\]
...
= 1-p  +2(p-1) = p-1.
\]
This relation is a difference of analytic functions on an open set, so by continuation it applies everywhere. Thus the effect of the covering transformation $\tilde{λ}$ on $\tilde{T}$ is to increase its value by $p-1$.
...
\[
(p,q,k,\tilde{x}) \mapsto (p, q + (p-1), k, \tilde{x}'), \text{ where } \tilde{x}' = 2π\Wind(\tilde{x}) + π + 2\atan (k \tan (\tilde{x}/2)).
\labelthis{eqn:group action}
\]
Thus the group of covering transformations preserve the level sets of $\tilde{T}$; specifically they provide an identification between the level set $\mathcal{\tilde{A}}_1(p,q)$ and the level set $\mathcal{\tilde{A}}_1(p,q + l(p-1))$ for any integer $l$.
...
\[
\{ (q,k,[\tilde{x}]_{\tilde{λ}}) \in \R \times (0,1) \times \bra{\R/π\Z} \},
\]
where $[\tilde{x}]_{\tilde{λ}}$ refers to the equivalence relation \eqref{eqn:annuli actions}, such that the subspace of spectral curves $\mathcal{S}_1(1)$ is
\[
\mathcal{S}_1(1) = \{ \mathcal{C}_1(1) \mid q \in \Q \}.
\]
...
\]
Focusing just on the $\tilde{x}$ component, this identifies any $\tilde{x} \in \R$ with a unique value in $[0,π)$ and so we may diffeomorphically identify $\R/\Z\langle\tilde{λ}\rangle$ with $\R/π\Z$. The result is that the quotient of each level set $\mathcal{\tilde{A}}_1(1,q)$ by the group $\Z\langle\tilde{λ}\rangle$ is an annulus parameterised by $(k,[\tilde{x}]_{\tilde{λ}}) \in (0,1)\times \R/π\Z$, where $[\tilde{x}]_{\tilde{λ}}$ refers to the equivalence relation \eqref{eqn:annuli action} from above
\end{proof}
\end{thm}
...
In the last section, we deduced the topology of the path components of the moduli space of spectral curves. Most components are strips, but the components of $\mathcal{S}_1(1)$ are distinguished because the are not simply connected. This section will show several properties of these components. First that although the moduli space of spectral curves is not simply connected, the moduli space of spectral data is simply connected. Second we will examine the special case of harmonic maps to a $2$-sphere and show that those with a genus one spectral case lie within a particular path component of $\mathcal{S}_1(1)$. Finally, we shall extend the extra symmetry exhibited by harmonic maps to the $2$-sphere to a transformation on the space spectral curves and give a geometric interpretation. We will illustrate this interpretation by observing how it applies to the space $\mathcal{S}_0$ of spectral curves with genus zero.
...
For $S=1$, take any path component of $\mathcal{S}_1(1)$. It is an annulus. Writing $S=1=n/m$ in as a reduced fraction implies that $m=n=1$. From \eqref{eqn:group action} it follows that $T = m'/n'$ is well defined and constant on the whole annulus. Consider a nontrivial loop $\ell : [0,1] \to \mathcal{S}_1(1)$ that wraps around the annulus once. If we make a choice of $γ_+,γ_-$ on each $Σ(\ell(t))$ such that the paths vary smoothly in $t$, what will be the change in $Ψ^P$ when we return to $\ell(1) = \ell(0)$? Let $Ψ^P = b Θ^E + m' Θ^P$ be the initial differential, and let $\tilde{Ψ}^P  = b' Θ^E + m' Θ^P$ be its continuation to $\ell(1)$. From our previous compuations, each of
\[
