...
With this differential found, every integral differential is of the form $a Θ^1 + n Θ^2$ for some $a\in\R$ and $n\in\Z$.
...
\todo{} These differenitals are also invariant under the involution that swaps the branch points, so they also serve as a basis for the differentials over $\mathcal{C}$. True by inspection for $Θ^1$, use above lemma for $Θ^2$
...
\int_{γ_+} Θ^2
= 4 E(k) F(f(1);k) &- 4 K(k) E(f(1);k) + 4\iu K \frac{w(\iu u)}{u-v}.
...
\int_{γ_-} Θ^2
= 4 E(k) F(f(-1);k) &- 4 K(k) E(f(-1);k) - 4\iu K \frac{w(\iu v)}{u-v}.\labelthis{eqn:gamma_minus2}
\end{align*}
And hence that
\begin{align*}
2π\iu T_0(p,k,u,v)
...
\end{align*}
Thus to do computations with the function $\tilde{T}$, one can continue to work with the function $T_0$ downstairs on $\mathcal{A}$ and make a note of the constants. In particular, $T \in \Q$ if and only $\tilde{T} \in \Q$, and so we can seek to find moduli space as a subset of the universal cover.
...
\begin{proof}
We prove this by direct application of the implict function theorem. Suppose first that $p \geq 1$. Consider the function $\tilde{T}(p,k,\tilde{u},\tilde{v})$ on $\mathcal{\tilde{A}}$. Consider the following formula for its derivative with respect to $\tilde{u}$.
\begin{align*}
...
% \]
% for $\tilde{v} \not\in 2π\Z$.
As witnessed in Lemma \ref{lem:deriv no zeroes}, $U$ is never zero, and neither are the other functions present. Hence $\partial \tilde{T} / \partial \tilde{u}$ is never zero on this open set. It remains to check it does not vanish where $\tilde{u} \in π + 2π\Z$. As $\tilde{T}$ is an analytic function, its derivatives are continuous and so we may compute their value at this point by taking a limit. The corresponding limit downstairs is $u\to\infty$. Observe
\begin{align*}
...
Therefore
\begin{align}
\lim_{\tilde{u}\to π + 2π\Z} \Partial{\tilde{T}}{\tilde{u}}
...
\end{align}
which is also nonzero. By the implicit function theorem therefore, there is a function $h$ such that the level set $\tilde{T}=q$ is a graph of the form $(p, k, \tilde{u}, h(p,k,\tilde{u}))$. Fixing a value of $p$ gives $\mathcal{\tilde{S}}(p,q)$ as a graph of the two remaining coordinates $(k,\tilde{u})$.
