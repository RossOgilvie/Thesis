...
\labelthis{eqn:def_T}
\]
using the definition of $S(α,β)$ to write $-η^+(1)/η^+(-1)$. We shall prove that these are sufficient conditions.
...
\end{align*}
Because later (Lemma \ref{lem:minimal differentials}) we will be interested in minimal solutions to these equations, we see that $l$ must be at least
\[
l = \frac{m'}{\gcd(m',mn')}.
\]
Take this to be the value of $l$. To then solve this equation for $Γ^P_+$ and $Γ^P_-$, as $n$ and $m$ are coprime let $x,y$ be integers such that $nx - my = 1$. By Bezout's Identity, the solution set is
\[
