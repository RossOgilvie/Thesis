...
%%%%%%%%%%%%%%%%%%%%
v &\multirow{2}{*}{Conformal: $P_0 = 0$}& 0 & 0,1
& $\gcd(b^1,b^2) \in \mathcal{P}^0_\R \cup \mathcal{P}^1_\R$, but $\gcd(b^1,b^2)\neq ζ$ \\ \cline{1-1}\cline{3-5}
vi && 1 & 0
& $\gcd(P,b^1,b^2) = \gcd(b^1,b^2) = ζ$ \\ \cline{1-1}\cline{3-5}
vii && \multicolumn{2}{|c|}{$\deg FG > 1$}
& The degree of $\gcd(b^1,b^2)$ is greater than one \\ \hline
\end{tabular}
\end{center}

We have seen that the spectral data in cases (iv) and (vii) do not admit any deformations.
...
As the points of cases (iii), (iv) and (vii) form a closed subvariety of the space of real polynomials, $U'$ is open. With reference to Definition \ref{def:def U}, we may characterise $U'$ as the set
\[
U' = \Set{ (P,b^1,b^2) \in U } { F = \gcd(P,b^1,b^2) = 1,\; G=\gcd(b^1,b^2) \in \mathcal{P}^0_\R \cup \mathcal{P}^1_\R \cup \mathcal{P}^2_\R }.
\labelthis{eqn:def U'}
\]
...
\]
The remainder of the chapter seeks to prove that $\mathcal{M}_g \cap U'$ is a manifold. We also show that the points of case (vi) are smooth points of $\mathcal{M}_g$.
...
In cases (ii) and (vi), the degree of $\tilde{b}^2$ is
\[
...
\begin{proof}
Recall the defintion of $U'$, equation \eqref{eqn:def U'}, as the open set whose points correspond to cases (i), (ii) or (vi). That is, $U'$ is the open set of points $(P,b^1,b^2)$ where $P$ has only simple roots, none of which are on the unit circle, the polynomials $b^i$ have at most a simple roots at $ζ=0$, $\gcd(P,b^1,b^2)$ is either one or $ζ$, and $\deg\left[ \gcd(b^1,b^2) \right] \leq 2 - \deg\left[ \gcd(P,b^1,b^2)\right]$.
