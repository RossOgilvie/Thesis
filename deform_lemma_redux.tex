...
Suppose that we are at a point of $\mathcal{M}_g$ that admits a deformation $\ell$. If we write $\ell(t) = (Σ(t),Θ^1(t),Θ^2(t))$, then we know from \eqref{eqn:def P} that every spectral curve $Σ(t)$ along this deformation may be written as $η^2 = P(t,ζ)$, for $P(t,ζ)$ a real polynomial of degree $2g+2$ in $ζ$, and from \eqref{eqn:def b} that each differential can be described by a real polynomial $b^i(t,ζ)$ of degree $g+3$ in $ζ$,
\[
