...
\hline
Case & Type & $F \in \mathcal{P}^k_\R$ & $G\in \mathcal{P}^l_\R$ & Description\\ \hline\hline
...
%%%%%%%%%%%%%%%%%%%%
v &\multirow{2}{*}{Conformal: $P_0 = 0$}& 0,2 & 0,1
& $\gcd(b^1,b^2) \in \mathcal{P}^0_\R \cup \mathcal{P}^1_\R \cup \mathcal{P}^2_\R$ \\ \cline{1-1}\cline{3-5}
vi && \multicolumn{2}{|c|}{$FG \in \mathcal{P}^k_\R,\, k>2$}
& $\gcd(b^1,b^2)\in \mathcal{P}^k_\R$ for some $k>2$ \\ \hline
\end{tabular}
\end{center}

These six cases are disjoint and cover all of the space $U$. We have seen that the spectral data in cases (iv) and (vi) do not admit any deformations.
...
Observe that because of the requirements that the differentials must have double poles, $\mathcal{M}_g$ will only intersect with a specific part of case (v). By equation REF, if $P_0$ vanishes at a point of $\mathcal{M}_g$ then so too must $b^1_0$ and $b^2_0$. Thus the intersection of $\mathcal{M}_g$ and the points of case (v) are the points where $\gcd(b^1,b^2) = ζ$. We will use this observation in Lemma \ref{lem:tangent conformal} to make introduce some assumptions on the polynomials $b^i$ without any loss of generality.

Let $U'$ be the subset of points of the three cases: (i), (ii) and (v). The set $U'$ is open because the points of cases (iii), (iv) and (vi) form a closed subset of the space $U$ of triples of real polynomials satifying the conditions of REF. To see that it is closed, first note that the union of cases (iv) and (vi) may be described simply as the space where $\gcd(b^1,b^2)$ lies in $\mathcal{P}^k_\R$ for some $k>2$, without regard for whether $P_0$ vanishes or not. Then the union of this with the points of case (iii) is the same as the union
\[
\Set{(P,b^1,b^2) \in U}{ \gcd(b^1,b^2) \in\mathcal{P}^k_\R,\,k >2 }
\cup
\Set{(P,b^1,b^2) \in U}{ \gcd(P,b^1,b^2) \in\mathcal{P}^l_\R,\,l \geq 2 },
\]
which is the union of two closed sets.

With reference to Definition \ref{def:def U}, we may characterise $U'$ as the set
\[
U' = \Set{ (P,b^1,b^2) \in U } { F = \gcd(P,b^1,b^2) \in \mathcal{P}^k_\R,\, k < 2, \text{ and } \gcd(b^1,b^2) \in \mathcal{P}^l_\R, l \leq 2 }.
\labelthis{eqn:def U'}
\]
...
\]
The remainder of the chapter seeks to prove that $\mathcal{M}_g \cap U'$ is a manifold. We also show that the points of case (v) are smooth points of $\mathcal{M}_g$.
...
\begin{proof}
Recall the defintion of $U'$, equation \eqref{eqn:def U'}, as the open set whose points correspond to cases (i), (ii) or (v). That is, $U'$ is the open set of points $(P,b^1,b^2)$ where $P$ has only simple roots, none of which are on the unit circle, the polynomials $b^i$ have at most a simple roots at $ζ=0$, $\gcd(P,b^1,b^2)$ is either one or $ζ$, and $\deg\left[ \gcd(b^1,b^2) \right] \leq 2 - \deg\left[ \gcd(P,b^1,b^2)\right]$.
