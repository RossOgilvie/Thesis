...
\[
(ρ^*s) (ζ)
= \bar{ζ}^{k} \bar{s\bra{ζ}^{-1}}
= \bar{s_0}\bar{ζ}^{k} + \bar{s_1} \bar{ζ}^{k-1} + \dots + \bar{s_k},
\]
hence a section is real is its coefficients obey $s_i = \bar{s}_{k-i}$, for $0 \leq i \leq k$. Because we shall often focus our attention on polynomials rather the sections to which they correspond, we define spaces of polynomials that a real with respect to $ρ$.
\begin{defn}
Let $\mathcal{P}^k$ be the space of polynomials of degree at most $k$. We define the real polynomials $\mathcal{P}^k_\R$ to be
\[
\mathcal{P}^k_\R
= \Set{ s = s_0 + \dots s_k ζ^k \in \mathcal{P}^k }{ s_i = \bar{s}_{k-i} \text{ for all } 0 \leq i \leq k}.
\]
\end{defn}
If one is unfamiliar with this style of reality structure, this definition may produce some counterintuitive examples. For example, the polynomial $ζ$ is a real polynomial of $\mathcal{P}^2$. On reflection however, this is entirely appropriate, as fundamentally it represents a section of $\mathcal{O}(2)$ and the lack of a quadratic term simply indicates the presence of a roots at $ζ = \infty$.
